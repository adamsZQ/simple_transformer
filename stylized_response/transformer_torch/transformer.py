...
def subsequent_mask(size):
    ...
    tgt_mask = np.triu(np.ones(attn_shape), k=1).astype('uint8')
    return torch.from_numpy(tgt_mask) == 0
...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, d_model, dropout, max_len=5000):
        ...
        # NOTE!! arange should use float type step to get a float tensor
...
def greedy_decode(model, src, src_mask, max_len, start_symbol):
    ...
    # FIXME it should be size of (batch_size, 1)
